...
export type AudioTick = (
  msFromRequestAnimationFrame: number,
  inputs: AudioInputs
) => void;
...
export function main(runAnalysis = false): AudioAPI {
  ...
  const stateOscillators = Array.from({ length: oscCount }, () => ({
    amplitude: 0,
    flicker: 0,
  }));
  ...
  function tick(ms: number, inputs: AudioInputs) {
    ...
    const detune =
      math.impulse((10 * detuneTime) / detuneDuration) -
      math.impulse((4 * detuneTime) / detuneDuration) +
      math.impulse((2 * detuneTime) / detuneDuration) -
      math.impulse(detuneTime / detuneDuration);
    state.detune = Math.abs(detune);

    // Active
    const oscActiveSin = Math.sin(
      (math.TAU * globalTime) / oscActiveCycleSeconds
    );
    const oscActiveCount = math.renormalized(
      oscActiveSin,
      -1,
      1,
      oscMinActive,
    ...
    const amplitudePulse =
      inputs.effects.doPulse * math.impulse(globalTime % pulseEveryNth);
    ...
    oscs.forEach((osc, oscIndex) => {
      // Frequency
      const freq =
        transRootFreq * getNoteInScale(currentChord, oscIndex) +
        math.rand(-chorus * blorpIntensity, chorus * blorpIntensity);
      ...
      const amplitudeActiveModulated =
        activeAmplitude * Math.sin(amplitudePhase) ** 20;
      const amplitude = Math.min(
        1,
        amplitudeActiveModulated +
          amplitudePulseScaled +
          amplitudeChorus +
          amplitudeDetune +
          amplitudeDistort
      );
      ...
      const coefPhase =
        (Math.PI * uniqueTime) / coefSurgeIntervalSeconds +
        (coefPhaseFlicker * oscIndex) / oscCount;
      ...
      setValue(
        osc.gainLow.gain,
        inputs.oscillators[oscIndex] * scaledAmplitude
      );
      setValue(
        osc.gainHigh.gain,
        inputs.flickers[oscIndex] * scaledAmplitude * scaledCoefIntensity
      );
    });
    ...
    const bassFreq =
      (transRootFreq * getNoteInScale(currentChord, bassNote)) / 3;
    ...
    bass.node1.type = math.arrMod(
      oscTypes,
      Math.floor(uniqueTime / bassTypeInterval)
    );
    bass.node2.type = math.arrMod(
      oscTypes,
      Math.floor(uniqueTime / bassTypeInterval)
    );
    ...
    const melodyTime =
      uniqueTime * math.arrMod(diatonic, Math.round(uniqueTime));
    const melodyNote =
      melodyNoteOffset +
      (Math.floor((uniqueTime + math.rand(-0.2, 0.2)) / 2) % melodyNoteCount);
    ...
    const melodyAmplitudeScaled =
      inputs.effects.doMelody * melodyAmplitude * 0.03;
    ...
    melody.node1.type = math.arrMod(
      oscTypes,
      Math.floor(uniqueTime / melodyTypeInterval)
    );
    melody.node2.type = math.arrMod(
      oscTypes,
      Math.floor(uniqueTime / melodyTypeInterval)
    );
    const melodyDetune =
      20 * Math.sin((Math.PI * uniqueTime) / melodyDetuneCycleSeconds) ** 4;
...
function makeOscs(count) {
  return Array.from({ length: count }, (_, i) => {
    ...
    const dir = i % 2 == 0 ? 1 : -1;
    const panLow = new StereoPannerNode(audio.context, {
      pan: math.renormalized(i, 0, count, 0, dir),
    });
    const panHigh = new StereoPannerNode(audio.context, {
      pan: math.renormalized(i, 0, count, 0, -dir),
    });
    ...
    nodeLow.setPeriodicWave(
      audio.context.createPeriodicWave(real, imag, {
        disableNormalization: true,
      })
    );

    real = real.map((_, coef) => (coef == 0 ? 0 : 1));
    nodeHigh.setPeriodicWave(
      audio.context.createPeriodicWave(real, imag, {
        disableNormalization: true,
      })
    );
