...
const distortionIntervalSeconds = 10; // Unique
...
export function main(runAnalysis = false): AudioAPI {
  ...
  function tick(ms: number, orientation: Orientation) {
    ...
    const blorpChorus = Math.max(
      0,
      Math.tan((Math.PI * uniqueTime) / blorbIntervalSeconds) ^ 5
    ); // Bitwise happy accident
    ...
    // Heavy distortion
    const distortion =
      Math.sin((Math.PI * uniqueTime) / distortionIntervalSeconds) ** 80;
    ...
    oscs.forEach((osc, oscIndex) => {
      ...
      const freq =
        ...
        math.rand(-chorus, chorus);
      setValue(osc.nodeLow.frequency, freq);
      setValue(osc.nodeHigh.frequency, freq);

      // Detune
      const detune =
        20 * Math.sin((math.TAU * globalTime) / detuneCycleSeconds);
